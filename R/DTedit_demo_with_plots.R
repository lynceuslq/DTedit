...
##### Create plot function to be added to dtedit function
publ <- function(dataframe, list) {
  with(list, {
    df <- subset(dataframe, Publisher==type) 
    ggplot() + geom_bar(data=df, mapping = aes(y=Date)) #define your plot function
    
  })
}

##### List your shiny widget to control plots in a R list
mywidgets <- list(list("selectInput", 
                       "type", 
                       "type to plot", 
...
if(!'books' %in% dbListTables(conn)) {
  books <- read.csv('/Library/Frameworks/R.framework/Versions/4.1/Resources/library/DTedit/shiny_demo/books.csv', stringsAsFactors = FALSE)
...
server <- function(input, output, session) {
  ...
  dtedit(input, output,
         name = 'books',
         thedata = books, 
         pltfunc = publ, #here is your plot function
         input.args = mywidgets, #here are the widgets for the plot
